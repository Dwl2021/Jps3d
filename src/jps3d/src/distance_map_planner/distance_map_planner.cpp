...
vec_Vecf<Dim> DMPlanner<Dim>::removeCornerPts(const vec_Vecf<Dim>& path)
{
  ...
  for (unsigned int i = 1; i < path.size() - 1; i++)
  {
    ...
    if (cost3 < cost1 + cost2)
    {
      ...
      // path[i](0), path[i](1), cost1, cost2, cost3);
    }
    else
    {
...
vec_Vecf<Dim> DMPlanner<Dim>::getSearchRegion()
{
  ...
  if (Dim == 2)
  {
    for (int nx = 0; nx < dim(0); nx++)
    {
      ...
      {
        int idx = nx + dim(0) * ny;
        if (search_region_[idx]) pts.push_back(map_util_->intToFloat(Veci<Dim>(nx, ny)));
      }
    }
  }
  else
...
template <int Dim>
std::shared_ptr<MapUtil<Dim>> DMPlanner<Dim>::getMapUtil()
{
  return map_util_;
}
...
bool DMPlanner<Dim>::checkAvailability(const Veci<Dim>& pn)
{
  ...
  {
    if (planner_verbose_) printf(ANSI_COLOR_RED "point is unknown!\n" ANSI_COLOR_RESET);
    return false;
  }
  ...
  {
    if (planner_verbose_) printf(ANSI_COLOR_RED "point is occupied!\n" ANSI_COLOR_RESET);
    return false;
  }
  return true;
}
  ...
  if (Dim == 2)
  {
    for (n(0) = -rn; n(0) <= rn; n(0)++)
    {
      for (n(1) = -rn; n(1) <= rn; n(1)++)
      {
        ...
        if (h > 1e-3) mask.push_back(std::make_pair(n, (int8_t)h));
      }
    }
  }
  else
  {
    ...
    for (n(0) = -rn; n(0) <= rn; n(0)++)
    {
      for (n(1) = -rn; n(1) <= rn; n(1)++)
      {
        ...
        {
          if (std::hypot(n(0), n(1)) > rn) continue;
          double h = h_max * std::pow((1 - (double)std::hypot(n(0), n(1)) / rn) *
...
template <int Dim>
void DMPlanner<Dim>::setMap(const std::shared_ptr<MapUtil<Dim>>& map_util,
                            const Vecf<Dim>& pos)
{
  map_util_ = std::make_shared<MapUtil<Dim>>(*map_util);
...
template <int Dim>
bool DMPlanner<Dim>::plan(const Vecf<Dim>& start, const Vecf<Dim>& goal, decimal_t eps,
                          decimal_t cweight)
{
  if (planner_verbose_)
  {
  ...
  {
    if (planner_verbose_) printf(ANSI_COLOR_RED "start is invalid!\n" ANSI_COLOR_RESET);
    status_ = 1;
    return false;
  }
  ...
  {
    if (planner_verbose_) printf(ANSI_COLOR_RED "goal is invalid!\n" ANSI_COLOR_RESET);
    status_ = 2;
    return false;
  }
  ...
  {
    graph_search_ = std::make_shared<DMP::GraphSearch>(cmap_.data(), dim(0), dim(1), eps,
                                                       cweight, planner_verbose_);
    path_cost_ = graph_search_->plan(start_int(0), start_int(1), goal_int(0), goal_int(1),
                                     search_region_);
  }
...
template <int Dim>
bool IterativeDMPlanner<Dim>::iterativeComputePath(const Vecf<Dim>& start,
                                                   const Vecf<Dim>& goal,
                                                   const vec_Vecf<Dim>& prior_path,
                                                   int max_iteration)
{
  if (this->planner_verbose_)
  {
    ...
    std::cout << "potential_map_range: " << this->potential_map_range_.transpose()
              << std::endl;
  }
