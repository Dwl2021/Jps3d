  ...
  public:
    ...
    void updateESDF3d()
    {
      ...
      for (int x = min_esdf[0]; x <= max_esdf[0]; x++)
      {
        for (int y = min_esdf[1]; y <= max_esdf[1]; y++)
        {
          ...
          fillESDF(
              [&](int z)
              {
                return isOccupied(toAddress(x, y, z)) ? 0 : std::numeric_limits<double>::max();
              },
              [&](int z, double val)
              { tmp_buffer1_[toAddress(x, y, z)] = val; },
              min_esdf[2],
              max_esdf[2], 2);
        }
      ...
      for (int x = min_esdf[0]; x <= max_esdf[0]; x++)
      {
        for (int z = min_esdf[2]; z <= max_esdf[2]; z++)
        {
          fillESDF([&](int y)
                   ...
                   [&](int y, double val)
                   { tmp_buffer2_[toAddress(x, y, z)] = val; },
                   min_esdf[1],
                   max_esdf[1], 1);
        }
      }
      ...
      for (int y = min_esdf[1]; y <= max_esdf[1]; y++)
      {
        ...
        {
          fillESDF([&](int x)
                   { return tmp_buffer2_[toAddress(x, y, z)]; },
                   [&](int x, double val)
                   {
