...
GraphSearch::GraphSearch(const char* cMap, int xDim, int yDim, int zDim, double eps,
                         bool verbose)
...
bool GraphSearch::plan(int xStart, int yStart, int xGoal, int yGoal, bool useJps,
                       int maxExpand)
{
  use_2d_ = true;
  pq_.clear();
...
bool GraphSearch::plan(StatePtr& currNode_ptr, int maxExpand, int start_id, int goal_id)
{
  ...
  while (true)
  {
    ...
    {
      if (verbose_) printf("MaxExpandStep [%d] Reached!!!!!!\n\n", maxExpand);
      return false;
    }
  ...
  else
  {
    for (const auto& d : ns_)
    {
      ...
      if (!seen_[new_id])
      {
        seen_[new_id] = true;
        hm_[new_id] =
            std::make_shared<State>(new_id, new_x, new_y, new_z, d[0], d[1], d[2]);
...
bool GraphSearch::jump(int x, int y, int dx, int dy, int& new_x, int& new_y)
{
  new_x = x + dx;
  new_y = y + dy;
  if (!isFree(new_x, new_y)) return false;

  if (new_x == xGoal_ && new_y == yGoal_) return true;
...
inline bool GraphSearch::hasForced(int x, int y, int dx, int dy)
{
  ...
  for (int fn = 0; fn < 2; ++fn)
  {
    ...
    if (isOccupied(nx, ny)) return true;
  }
  return false;
}
...
JPS2DNeib::JPS2DNeib()
{
  ...
  for (int dy = -1; dy <= 1; ++dy)
  {
    for (int dx = -1; dx <= 1; ++dx)
    {
      ...
      {
        FNeib(dx, dy, norm1, dev, f1[id][0][dev], f1[id][1][dev], f2[id][0][dev],
              f2[id][1][dev]);
      }
      id++;
    }
...
void JPS2DNeib::FNeib(int dx, int dy, int norm1, int dev, int& fx, int& fy, int& nx,
                      int& ny)
{
  switch (norm1)
  {
...
void JPS3DNeib::Neib(int dx, int dy, int dz, int norm1, int dev, int& tx, int& ty,
                     int& tz)
{
  switch (norm1)
  {
...
void JPS3DNeib::FNeib(int dx, int dy, int dz, int norm1, int dev, int& fx, int& fy,
